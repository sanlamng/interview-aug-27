...
<table cellspacing="0" cellpadding="0" class="tablemain" style="margin-top: 50px;">
    ...
    @foreach($transactions as $transaction)
        <tr>
            <td class="border-top border-right width70">{{$transaction->product->name}}</td>
            <td class="border-top width30">{{$transaction->amount}}</td>
        </tr>
    @endforeach
    <tr>
        <td class="border-top border-right width70"></td>
        <td class="border-top width30">{{'N'.number_format($transactions->sum('amount_paid'), 2)}}</td>
    </tr>
    </tbody>
</table>
</body>
